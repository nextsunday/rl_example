...
best_model = atari_model(n_actions=4, ATARI_SHAPE=ATARI_SHAPE)

# model = load_model('./atari_keras128000.h5')  # 추가학습시
# best_model = load_model('./atari_keras128000.h5')  # 추가학습시

e = START_EXPLORATION = 1.0
N_ITERATION = 1000000
MEMORY_SIZE = 100000
BATCH_SIZE = 32
...
for i in range(N_ITERATION):
    ...
    while not is_done:
        ...
        if len(replay_memory) >= TRAIN_START:  # Train Start (DQN Update)
            mini_batch = random.sample(replay_memory, BATCH_SIZE)
            history = fit_batch(model=best_model, gamma=gamma, mini_batch=mini_batch)
            if steps % 100 == 0:
    ...
    if i % NETWORK_UPDATE_TIME == 0 and i > 0:
